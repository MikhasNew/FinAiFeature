            ...
            await Task.Run(() =>
            {
                ...
                foreach (Sms sms in smslist)
                {
                   // Thread.Sleep(10000);
                    ...
                    var config = BankConfigurations.First(x => x.SmsNumber == sms.getAddress());

                    if (config.PaymentTemplates == null)
                        throw new InvalidOperationException(
                            $"PaymentTemplates is null. SmsNumber={config.SmsNumber}");

                    if (config.DepositTemplates == null)
                        throw new InvalidOperationException(
                            $"DepositTemplates is null. SmsNumber={config.SmsNumber}");

                    if (config.CashTemplates == null)
                        throw new InvalidOperationException(
                            $"CashTemplates is null. SmsNumber={config.SmsNumber}");

                    if (string.IsNullOrEmpty(parselableOperTyp))
                        throw new InvalidOperationException(
                            $"Parsed operation type is null or empty. SmsNumber={config.SmsNumber}");

                    if (config.PaymentTemplates?.Contains(parselableOperTyp) == true)
                        operType = OperacionTyps.OPLATA;
                    else if (config.DepositTemplates?.Contains(parselableOperTyp) == true)
                        operType = OperacionTyps.ZACHISLENIE;
                    else if (config.CashTemplates?.Contains(parselableOperTyp) == true)
